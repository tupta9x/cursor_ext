...
function openChromeCatalog() {
  ...
  if (dontshow.checked) localStorage.setItem("dontshow", 1);
  ovelay.style.display = "none";
  votepls.style.display = "none";
  window.open(
    "https://chrome.google.com/webstore/detail/cursorland-custom-cursor/oinkhgpjmeccknjbbccabjfonamfmcbn/reviews"
  );
}
function starClose() {
  localStorage.setItem("visit_count", 0);
...
document
  .querySelector("#star_close_btn")
  .addEventListener("click", function () {
    starClose();
  });
document
  .querySelector("#star_img_container")
  .addEventListener("click", function () {
    openChromeCatalog();
  });
